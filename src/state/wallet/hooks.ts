...
export function useCurrencyBalances(
  ...
  const containsCRO: boolean = useMemo(
    () => currencies?.some((currency) => currency === CRO) ?? false,
    [currencies]
  );
  const croBalance = useCROBalances(containsCRO ? [account] : []);
  ...
  return useMemo(
    () =>
      currencies?.map((currency) => {
        ...
        if (currency === CRO) return croBalance[account];
        return undefined;
      }) ?? [],
    [account, currencies, croBalance, tokenBalances]
  );
}
