...
import { Currency, CRO } from "./currency";
...
import { currencyEquals, Token, WCRO } from "./token";
 ...
 * In other words, if the currency is CRO, returns the WCRO token amount for the given chain. Otherwise, returns
...
function wrappedAmount(
  ...
  if (currencyAmount.currency === CRO)
    return new TokenAmount(WCRO[chainId], currencyAmount.raw);
  invariant(false, "CURRENCY");
}

function wrappedCurrency(currency: Currency, chainId: ChainId): Token {
  if (currency instanceof Token) return currency;
  if (currency === CRO) return WCRO[chainId];
  invariant(false, "CURRENCY");
}
